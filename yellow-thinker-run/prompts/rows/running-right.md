Create a single horizontal sprite strip for the Codex app digital pet `yellow-thinker` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: small codex-style desktop pet, based on the attached yellow creature concept sheet, preserve the rounded body, green eyes, tiny mouth, calm awkward charm, and thoughtful personality.
