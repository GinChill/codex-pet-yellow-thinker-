Create a single horizontal sprite strip for the Codex app digital pet `yellow-thinker` in the state `waiting`.
...
Animation action: sleeping loop. The pet is clearly asleep in all frames.

State-specific requirements:
- Every frame must read as sleeping at a glance: curled or resting pose, eyes closed, calm breathing rhythm.
- Keep the body low to the ground as if napping; no alert upright posture.
- Small attached sleep cue is allowed only if touching/overlapping the pet silhouette.
- Do not depict waiting, alertness, waving, running, working, or thinking.
